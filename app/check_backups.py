...
def get_proxmox_connection(config):
    ...
    if user and token_name and token_secret:
        ...
        print("Fehler: Es wurden keine gültigen API-Token-Zugangsdaten für die Proxmox API gefunden. Bitte prüfe die config.json und gib einen API-Token an.")
        exit(1)
...
def get_last_scheduled_datetime(now, dow_list, starttime):
    ...
    for tage_zurueck in range(0, 8):
        ...
        if proxmox_tag in dow_indices:
            dt = (now - timedelta(days=tage_zurueck)).replace(hour=stunden, minute=minuten, second=0, microsecond=0)
            if dt <= now:
                return dt
    return None
...
def get_next_scheduled_datetime(now, dow_list, starttime):
    ...
    for tage_voraus in range(0, 8):
        ...
        proxmox_tag = (tag + 1) % 7  # 0=Sonntag
        if proxmox_tag in dow_indices:
            dt = (now + timedelta(days=tage_voraus)).replace(hour=stunden, minute=minuten, second=0, microsecond=0)
            if dt > now:
                return dt
    return None
...
def analyze_backup_logs(jobs, log_dir, vm_names, debug=False):
    ...
    for job in jobs:
        ...
        if debug:
            print(f"[DEBUG] Job {job_id}: enabled={enabled}, schedule='{schedule}' -> dow={dow_list}, starttime='{starttime}', vmids={job.get('vmid') or job.get('vmids')}")
    ...
    for vmid in alle_vmids:
        ...
        if debug:
            print(f"[DEBUG] VM/CT {vmid} - Job-Parameter: dow={dow_list}, starttime={starttime}")
        ...
        if os.path.exists(log_path_lxc):
            logfiles.append((log_path_lxc, datetime.fromtimestamp(os.path.getmtime(log_path_lxc))))
        if os.path.exists(log_path_qemu):
            logfiles.append((log_path_qemu, datetime.fromtimestamp(os.path.getmtime(log_path_qemu))))
        ...
        with open(best_log, 'r', encoding='utf-8') as log_file:
            ...
            hat_fehler = any(err in log_content for err in ['ERROR', 'FAILED', 'aborted'])
        ...
        if last_scheduled and best_mtime < last_scheduled:
            fehlerhafte_backups.append((vmid, f'Logdatei ist zu alt (letzte Änderung: {best_mtime}, erwartet: nach {last_scheduled})'))
        elif hat_fehler:
            fehlerhafte_backups.append((vmid, f'Fehler im Log gefunden ({os.path.basename(best_log)})'))
        else:
            erfolgreiche_backups.append((vmid, f'Backup erfolgreich am {best_mtime} ({os.path.basename(best_log)})'))
    ...
    if debug:
        ...
        for vmid, (mtime, logfile) in letzte_backups.items():
            ...
            print(f"  - VM/CT-ID: {vmid} ({vm_name}): {mtime} ({os.path.basename(logfile)})")
